...
export function fd(inp?: moment.MomentInput, format?: moment.MomentFormatSpecification, strict?: boolean): string {
  return moment(inp, format, strict).format('YYYY-MM-DD');
}
/**
 * 根据时区修改时间
 * @param {String} time
...
export function parseUrlParams<T = {}>(): T {
  // if (!window) return {};
  const qsParams: T = window.location.search.length ? qs.parse(window.location.search.substring(1)) as any : {};
