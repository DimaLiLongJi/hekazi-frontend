...
const storeEnhancers: any = compose(
  ...
  (root && (root as any).devToolsExtension) ? (root as any).devToolsExtension() : ((f: any) => f),
);

const store: any = createStore(combineReducers(reducer), (root as any).__INITIAL_STATE__, storeEnhancers);
